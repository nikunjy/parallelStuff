...
void for_each(IterTy b, IterTy e, FunctionTy fn, const char* loopname = 0) {
  GaloisRuntime::for_each_impl<WLTy>(GaloisRuntime::makeStandardRange(b, e), fn, loopname);
}
 ...
 * is the type of item.
 *
 * @tparam WLTy Worklist policy {@link GaloisRuntime::WorkList}
...
void for_each_local(ConTy& c, FunctionTy fn, const char* loopname = 0) {
  GaloisRuntime::for_each_impl<WLTy>(GaloisRuntime::makeLocalRange(c), fn, loopname);
}
...
FunctionTy do_all(const IterTy& b, const IterTy& e, FunctionTy fn, const char* loopname = 0) {
  return GaloisRuntime::do_all_impl(GaloisRuntime::makeStandardRange(b, e), fn, GaloisRuntime::EmptyFn(), false);
}
...
FunctionTy do_all_local(ConTy& c, FunctionTy fn, const char* loopname = 0) {
  return GaloisRuntime::do_all_impl(GaloisRuntime::makeLocalRange(c), fn, GaloisRuntime::EmptyFn(), false);
}
...
void for_each_ordered(Iter b, Iter e, Cmp cmp, NhFunc nhFunc, OpFunc fn, const char* loopname=0) {
  GaloisRuntime::for_each_ordered_impl(b, e, cmp, nhFunc, fn, loopname);
}
...
void for_each_ordered(Iter b, Iter e, Cmp cmp, NhFunc nhFunc, OpFunc fn, StableTest stabilityTest, const char* loopname=0) {
  GaloisRuntime::for_each_ordered_impl(b, e, cmp, nhFunc, fn, stabilityTest, loopname);
}
