...
protected:
  ...
  void createInitEvents (bool createNullEvents=true) {
    ...
    for (std::vector<BaseSimObj_ty*>::const_iterator i = inputObjs.begin (), ei = inputObjs.end (); i != ei; ++i ) {
      ...
      for (std::vector< std::pair<SimTime, LogicVal> >::const_iterator j = tvList.begin ()
          , ej = tvList.end (); j != ej; ++j) {

        const std::pair<SimTime, LogicVal>& p = *j;
        LogicUpdate lu(impl.getInputName(), p.second);

        Event_ty e = currInput->makeEvent(currInput, lu, Event_ty::REGULAR_EVENT, p.first);
      ...
      if (createNullEvents) {
        ...
        Event_ty fe = currInput->makeEvent (currInput, lu, Event_ty::NULL_EVENT, INFINITY_SIM_TIME);
   ...
   */
  template <typename G>
  static void createGraphNodes (const std::vector<BaseSimObj_ty*>& simObjs, G& graph, size_t& numNodes) {
  ...
  void createConnections (G& graph) {
    ...
    for (typename G::iterator i = graph.begin (), ei = graph.end (); i != ei; ++i) {
    ...
    for (typename std::vector<GNode>::iterator i = allNodes.begin (), ei = allNodes.end (); i != ei; ++i) {
      ...
      SimGate_tp* sg = dynamic_cast<SimGate_tp*> (graph.getData (src, Galois::NONE));
      assert (sg != NULL);
      ...
      for (typename std::vector<GNode>::iterator j = allNodes.begin (), ej = allNodes.end (); j != ej; ++j) {
        GNode& dst = *j;
        SimGate_tp* dg = dynamic_cast<SimGate_tp*> (graph.getData (dst, Galois::NONE));
        assert (dg != NULL);
...
public:
  ...
  void verify () const {
    ...
    for (typename std::vector<BaseSimObj_ty*>::const_iterator i = outputObjs.begin ()
        , ei = outputObjs.end (); i != ei; ++i) {
      BaseSimObj_ty* so = *i;
    ...
    if (exitStatus != 0) {
      ...
      for (typename std::vector<BaseSimObj_ty*>::const_iterator i = outputObjs.begin ()
        ...
        std::cerr << "expected: " << expected << ", " << outObj->str () << std::endl;
      }
   ...
   */
  const std::vector<Event_ty>& getInitEvents() const {
    return initEvents;
  }
   ...
   */
  const std::vector<BaseSimObj_ty*>& getInputObjs() const {
    return inputObjs;
  }
   ...
   */
  const std::vector<BaseSimObj_ty*> getOutputObjs() const {
    return outputObjs;
  }
